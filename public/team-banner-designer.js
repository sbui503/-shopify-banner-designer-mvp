(function () {
  ...
  const SCHOOL_SPORT_GENERATED_VALUES = ["basketball", "football", "volleyball", "track", "baseball", "softball", "soccer"];
  ...
  const SCHOOL_SPORT_PHOTO_FRAME_VARIANTS = [
    ...
    { key: "varsity", name: "Varsity Photo Frame" },
    { key: "champion", name: "Champion Photo Frame" },
    { key: "wings", name: "Wings Photo Frame" },
    { key: "halo", name: "Halo Photo Frame" },
    { key: "stadium", name: "Stadium Photo Frame" },
    { key: "pennant", name: "Pennant Photo Frame" }
  ];
  ...
  function normalizeShape(shape, hasDesign) {
    ...
    if (/home(baseball|softball|soccer|basketball|football|volleyball|track)?banners?/.test(value)) return "homeplatepennant";
    if (/homeplate|plate/.test(value)) return "homeplatepennant";
    if (/triangle|pennant/.test(value)) return "triangle";
    if (/banner|banne|rect|rectangle|soccer|baseball|softball|basketball|football|volleyball|track|field|hem|grommet|polepocket/.test(value)) return "rectangle";
    return "rectangle";
  }
  ...
  function sportBackgroundSvg(sport, type = "rectangle", variantKey = "arena") {
    ...
    const height = type === "rectangle" || type === "polepocket" ? 900 : 900;
  ...
  function sportPhotoFrameSvg(sport, variantKey = "classic") {
    ...
    const starRing = Array.from({ length: 10 }, (_, index) => {
      ...
      return `<path id="frame_star_${index + 1}" d="M${x} ${y - 18} l6 13 14 2 -10 9 3 14 -13 -7 -13 7 3 -14 -10 -9 14 -2z" fill="${index % 2 ? sport.primary : sport.glow}" stroke="${sport.accent}" stroke-width="4"/>`;
    }).join("");
    ...
    const decor = variant.key === "ribbon"
      ? `<path id="frame_ribbon_left" d="M112 448 L252 410 L252 610 L112 650 L150 548 Z" fill="${sport.accent}" stroke="#fff" stroke-width="10"/><path id="frame_ribbon_right" d="M648 448 L508 410 L508 610 L648 650 L610 548 Z" fill="${sport.accent}" stroke="#fff" stroke-width="10"/>`
      : variant.key === "star"
        ? starRing
        : variant.key === "shield"
          ? `<path id="frame_shield_back" d="M380 392 L604 468 L552 668 L380 742 L208 668 L156 468 Z" fill="${sport.accent}" stroke="#fff" stroke-width="14"/><path id="frame_shield_accent" d="M224 512 H536" stroke="${sport.primary}" stroke-width="18" stroke-linecap="round"/>`
          : variant.key === "varsity"
            ...
            : variant.key === "champion"
              ? `<path id="frame_crown" d="M242 114 L300 58 L356 118 L410 58 L462 118 L520 58 L576 114 L548 176 H270 Z" fill="${sport.glow}" stroke="${sport.accent}" stroke-width="10"/><path id="frame_champion_badge" d="M380 378 L620 470 L550 668 L380 738 L210 668 L140 470 Z" fill="${sport.accent}" stroke="#fff" stroke-width="14"/>`
              : variant.key === "wings"
                ? `<path id="frame_wing_left_1" d="M338 390 C210 388 118 330 56 232 C148 264 220 268 304 244" fill="#fff" opacity=".94" stroke="${sport.accent}" stroke-width="8"/><path id="frame_wing_left_2" d="M316 432 C200 444 110 408 36 326 C130 340 204 334 286 300" fill="${sport.glow}" opacity=".9" stroke="${sport.accent}" stroke-width="7"/><path id="frame_wing_right_1" d="M422 390 C550 388 642 330 704 232 C612 264 540 268 456 244" fill="#fff" opacity=".94" stroke="${sport.accent}" stroke-width="8"/><path id="frame_wing_right_2" d="M444 432 C560 444 650 408 724 326 C630 340 556 334 474 300" fill="${sport.glow}" opacity=".9" stroke="${sport.accent}" stroke-width="7"/>`
                : variant.key === "halo"
    ...
    return `<svg xmlns="http://www.w3.org/2000/svg" width="760" height="760" viewBox="0 0 760 760" data-photo-frame-variant="${variant.key}">
      <title>${escapeHtml(sport.label)} ${escapeHtml(variant.name)}</title>
      <defs>
        ...
        <linearGradient id="nameplate_gradient" x1="0" x2="1"><stop offset="0" stop-color="${sport.accent}"/><stop offset=".52" stop-color="${sport.primary}"/><stop offset="1" stop-color="${sport.accent}"/></linearGradient>
        <filter id="shadow" x="-20%" y="-20%" width="140%" height="140%"><feDropShadow dx="0" dy="10" stdDeviation="7" flood-color="#000" flood-opacity=".38"/></filter>
      </defs>
      <g id="player_photo_frame" filter="url(#shadow)">
        ...
        <circle id="player_frame_inner" cx="380" cy="286" r="178" fill="none" stroke="${sport.accent}" stroke-width="8"/>
        <g id="sport_icon" transform="translate(380 600)">${sportBallSvg(sport, 0, 0, 58)}</g>
        ${nameplate}
      </g>
    </svg>`;
  }
  ...
  function sportLabelFromContext(context) {
    return schoolSportLabel(detectSchoolSport(context) || "soccer").toUpperCase();
  }
  ...
  function readLaunchParams(root) {
    ...
    return {
      ...
      generatedAssetMapUrl: get("generatedAssetsUrl") || get("generatedAssetMapUrl"),
      generatedTemplateMapUrl: get("generatedTemplatesUrl") || get("generatedTemplateMapUrl"),
  ...
  async function init(root) {
    ...
    const els = {
      ...
      cartCurrentPreview: root.querySelector("[data-tbd-cart-current-preview]"),
      cartCurrentImage: root.querySelector("[data-tbd-cart-current-image]"),
      cartCurrentMeta: root.querySelector("[data-tbd-cart-current-meta]"),
    ...
    function generatedAssetRole(assetType, category) {
      const type = String(assetType || "").toLowerCase();
      if (type.includes("background")) return "background";
      if (type.includes("logo")) return "team-logo";
      if (type.includes("photo_frame")) return "photo-frame";
      if (category === PHOTO_FRAME_CATEGORY) return "photo-frame";
      if (category === "Team name") return "team-logo";
      if (category === "Clip art") return "clipart";
      return "player-icon";
    }

    function normalizeGeneratedAssetRecord(record) {
      const rawUrl = record.url || record.url_path || record.public_url || "";
      const url = rawUrl || (record.file_path ? `/${String(record.file_path).replace(/^public\//, "")}` : "");
      const category = record.design_tool_category || record.category || assetCategoryFromText([record.asset_type, record.name, url].join(" "));
      return normalizeAsset({
        name: record.name || record.alt_text || record.file_name || "Generated sports asset",
        category,
        url,
        svgUrl: record.svgUrl || url,
        sourceType: record.source_type || "generated-native-svg",
        rawType: record.asset_type || "",
        sourceId: record.id || record.file_name || url,
        role: generatedAssetRole(record.asset_type, category),
        matchKey: [
          record.sport,
          record.sport_slug,
          record.school_name,
        ...
        assetTags: [
          "tbd:generated-native-svg",
          record.sport_slug ? `tbd:sport:${record.sport_slug}` : "",
          record.usage_status ? `tbd:usage:${record.usage_status}` : "",
          category ? `tbd:asset-category:${category.toLowerCase().replace(/[^a-z0-9]+/g, "-")}` : ""
        ].filter(Boolean),
        schoolName: record.school_name || "",
        usageStatus: record.usage_status || ""
      });
    }

    function mergeAssetLists(baseList, additions) {
      const merged = (Array.isArray(baseList) ? baseList : []).map(normalizeAsset);
      const seen = new Set(merged.map((asset) => String(asset.url || asset.sourceId || "")));
      (Array.isArray(additions) ? additions : []).map(normalizeAsset).forEach((asset) => {
        const key = String(asset.url || asset.sourceId || "");
    ...
    function withPreloadedPhotoFrameAssets(list) {
      return mergeAssetLists(list, [...PHOTO_FRAME_ASSETS, ...SCHOOL_SPORT_ASSETS]);
    }
    ...
    function normalizeSvgTemplateEntry(template, index = 0) {
      ...
      return {
        ...
        type,
        sport: SCHOOL_SPORT_VALUES.includes(sport) ? sport : "",
        playerCount: Number(template.playerCount || template.players || 0) || 0
      };
    }

    function mergeSvgTemplateLists(baseList, additions) {
      const merged = (Array.isArray(baseList) ? baseList : []).map(normalizeSvgTemplateEntry).filter((template) => template.url);
      const seen = new Set(merged.map((template) => String(template.url || template.name || "")));
      (Array.isArray(additions) ? additions : []).map(normalizeSvgTemplateEntry).forEach((template) => {
        const key = String(template.url || template.name || "");
        if (!key || seen.has(key) || !template.url) return;
        ...
        for (const entry of parsed.imageEntries) {
          ...
          try {
            ...
            if (playerIconNumber && !effectiveOptions.usePhotoFrame) {
              ...
              addPlayerNumberTextLayer(effectiveOptions, playerIconNumber, {
                left: box.centerX,
                top: box.top + box.height * 0.78,
                fontSize: Math.max(14, Math.min(32, box.height * 0.28))
              });
            }
          } catch (error) {
    ...
    function photoFramePlacementFromTextBox(box) {
      ...
      const size = Math.max(
        76,
        Math.min(
          bounds.width * (isRectangularShape(ARTBOARD_SHAPE) ? 0.085 : 0.13),
          Math.max(box.width * 0.92, box.height * 2.35)
        )
      );
      return {
        left: box.centerX,
    ...
    async function loadGeneratedAssetMap() {
      const url = launch.generatedAssetMapUrl || root.dataset.generatedAssetsUrl;
      if (!url) return;
      try {
        const response = await fetch(resolveSourceUrl(url), { credentials: "omit" });
        if (!response.ok) throw new Error("Generated asset map request failed");
        const data = await response.json();
        const records = Array.isArray(data.assets) ? data.assets : Array.isArray(data) ? data : [];
        const generatedAssets = records.map(normalizeGeneratedAssetRecord).filter((asset) => asset.url);
        assets = mergeAssetLists(assets, generatedAssets);
      } catch (error) {
        // Supplemental generated assets are optional; legacy manifests remain authoritative.
      }
    }
    ...
    function setCartPanelOpen(open = true) {
      ...
      if (open) updateCartCurrentPreview();
      updateStageScale();
    }

    function updateCartCurrentPreview() {
      if (!els.cartCurrentPreview || !els.cartCurrentImage) return;
      try {
        const preview = exportDesign(0.22).image;
        els.cartCurrentImage.src = preview;
        if (els.cartCurrentMeta) {
          const shape = defaultHeadlineForShape(ARTBOARD_SHAPE);
          const objects = visibleLayerObjects().filter((obj) => obj && obj.data && obj.data.role !== "svg-layout-guide").length;
          els.cartCurrentMeta.textContent = `${shape} · ${objects} editable layer${objects === 1 ? "" : "s"}`;
        }
        els.cartCurrentPreview.hidden = false;
      } catch (error) {
        els.cartCurrentPreview.hidden = true;
      }
    }

    function renderDesignCart() {
      const count = designCart.length;
      updateCartCurrentPreview();
      if (els.cartCount) {
    ...
    function fitPhotoFrameNameTextLayer(textLayer, placement) {
      textLayer.set({ scaleX: 1, scaleY: 1, fontSize: placement.fontSize, strokeWidth: Math.max(1, placement.fontSize * 0.035) });
    ...
    function ensurePhotoFrameNameLayer(frame, options = {}) {
      ...
      if (!textLayer) {
        textLayer = new fabric.IText(nameText, {
          ...
          fontWeight: 900,
          stroke: "#07112f",
          strokeWidth: Math.max(1, placement.fontSize * 0.035),
          paintFirst: "stroke",
          shadow: "none",
          textAlign: "center",
          data: {
    ...
    async function loadSvgTemplates() {
      ...
      if (!els.svgTemplates && !els.generatorSvg) return;
      try {
        let loadedTemplates = [];
        if (url) {
          const response = await fetch(url, { credentials: "omit" });
        ...
        svgTemplates = mergeSvgTemplateLists([], loadedTemplates);
        await loadGeneratedTemplateMap();
        renderSvgTemplates();
    ...
    async function loadGeneratedTemplateMap() {
      const url = launch.generatedTemplateMapUrl || root.dataset.generatedTemplatesUrl;
      if (!url) return;
      try {
        const response = await fetch(resolveSourceUrl(url), { credentials: "omit" });
        if (!response.ok) throw new Error("Generated template map failed.");
        const data = await response.json();
        const records = Array.isArray(data.templates) ? data.templates : Array.isArray(data) ? data : [];
        svgTemplates = mergeSvgTemplateLists(svgTemplates, records);
      } catch (error) {
        // Supplemental generated templates are optional and never replace legacy templates.
      }
    }
